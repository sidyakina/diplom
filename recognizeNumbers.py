...
yml_file = open("nn_model.yml", "r")
loaded_model_yml = yml_file.read()
yml_file.close()
loaded_model = model_from_yaml(loaded_model_yml)
loaded_model.load_weights("nn_model.h5")
loaded_model.compile(loss="categorical_crossentropy", optimizer="SGD", metrics=["accuracy"])


numbers = loaded_model.predict(dataset[:, :])
f1 = []
for j in range(10):
    f1.append(numbers[0][j])
# print(f1)
# print(f1.index(max(f1)))

f2 = []
for j in range(10):
    f2.append(numbers[1][j])
# print(f2)
# print(f2.index(max(f2)))
if numbers.shape[0] == 3:
    f3 = []
    for j in range(10):
        f3.append(numbers[2][j])
    # print(f3)
    # print(f3.index(max(f3)))
    stringCode = str(f1.index(max(f1))) + str(f2.index(max(f2))) + str(f3.index(max(f3)))
else:
    stringCode = str(f1.index(max(f1))) + str(f2.index(max(f2)))


codeRegions = {'01': 'Республика Адыгея',
...
print("Мы распознали код региона: ", stringCode)
print("Этот регион ", codeRegions.get(stringCode, "не существует"))